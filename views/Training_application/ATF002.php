	...
	$('#staff_training_application').on('click', '.sta_appsm_btn', function() {
		...
		$.confirm({
		    ...
		    buttons: {
					...
					$.ajax({
						...
						success: function(res) {
							if (res.sts==1) {
								...
								$('#myModalis #mContent').empty();
								$('#myModalis').modal('show');
								$('#myModalis').find('#mContent2').html('<center><i class="fa fa-spinner fa-spin fa-3x fa-fw" style="color:black"></i></center>');
		        ...
		        cancel: function () {
		            $.alert('Canceled Approve & send Memo!');
		        }
		    }
		});
	});
